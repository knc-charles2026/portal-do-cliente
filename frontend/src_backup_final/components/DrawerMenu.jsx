...
const handleConfirmDelete = async () => {
  try {
    await del("`http://localhost:8001/oportunidades/${deleteId}`");
...
const handleDelete = async (id) => {
  if (window.confirm("Deseja realmente excluir esta oportunidade?")) {
    try {
      await del("`http://localhost:8001/oportunidades/${id}`");
      ...
      <Tooltip title="Duplicar" placement="left">
        <IconButton
          ...
          onClick={async (e) => {
            e.stopPropagation();
            try {
              await post("`http://localhost:8001/oportunidades/${params.row.id}/duplicar`");
              fetchOportunidades();
      ...
      <FormOportunidade
        ...
        onSalvar={async () => {
          try {
            ...
            if (modoForm === "update") await put("`http://localhost:8001/oportunidades/${registroSelecionado?.id}/`", formData);
            if (modoForm === "delete") await del("`http://localhost:8001/oportunidades/${registroSelecionado?.id}/`");
            setModo("list");
            setModoForm(null);
            setRegistroSelecionado(null);
            setFormTipo(null);
