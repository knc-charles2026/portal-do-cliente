...
export default function CadastroOportunidades({ onLogout }) {
  ...
  const [rowHeight, setRowHeight] = useState(30);  // Default (52)
...
const handleConfirmDelete = async () => {
  try {
    await axios.delete(`http://localhost:8001/oportunidades/${deleteId}`);
...
const handleDelete = async (id) => {
  if (window.confirm("Deseja realmente excluir esta oportunidade?")) {
    try {
      await axios.delete(`http://localhost:8001/oportunidades/${id}`);
  ...
  const fetchOportunidades = async () => {
    try {
      const response = await axios.get('http://localhost:8001/oportunidades/');
...
useEffect(() => {
  ...
  const columns = useMemo(() => [
    //{ field: "id", headerName: "Nº RO",  minWidth: 90,fontWeight: "bold", textAlign: "center", cellClassName: "idCell"  },
    {
     ...
     width: 120,
    renderCell: (params) => (
      <span style={{ color: "#4f2f93ff", fontWeight: "bold" }}>
       {params.value}
      </span>
        ), 
    },
      ...
      <Tooltip title="Duplicar" placement="left">
        <IconButton
          ...
          onClick={async (e) => {
            ...
            try {
              await axios.post(
                `http://localhost:8001/oportunidades/${params.row.id}/duplicar`
              );
              fetchOportunidades();
              setSnackbarMessage("Registro duplicado com sucesso!");
              setSnackbarOpen(true); // ✅ ativa Snackbar
            } catch (error) {
              console.error("Erro ao duplicar:", error);
              setSnackbarMessage("Não foi possível duplicar a oportunidade.");
      ...
      >
        ...
        <div className="relative mb-4">
          <input
            ...
            required
            placeholder=" "
            className="peer w-full p-3 border rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <label
            htmlFor="usuario"
            className={`absolute left-3 top-3 text-gray-400 text-sm transition-all 
                        ${nomeUsuario ? "-top-2 text-xs text-blue-500" : "peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-400"}`}
          >
            Usuário
          </label>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <label
            htmlFor="senha"
            className={`absolute left-3 top-3 text-gray-400 text-sm transition-all 
                        ${senha ? "-top-2 text-xs text-blue-500" : "peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-400"}`}
          >
            Senha
          </label>
        </div>
        ...
        <button
          type="submit"
          className="w-full py-3 bg-[#1e293b] text-white font-medium rounded-lg hover:bg-blue-700 transition-colors"
        >
          Entrar
        </button>
      </form>
...
{/*           <ListItem button onClick={() => setUsuario(null)}>
    ...
    {formTipo === "oportunidades" && (
      <FormOportunidade
        ...
        onSalvar={async () => {
          try {
            if (modoForm === "create") await axios.post("http://localhost:8001/oportunidades/", formData);
            if (modoForm === "update") await axios.put(`http://localhost:8001/oportunidades/${registroSelecionado?.id}/`, formData);
            if (modoForm === "delete") await axios.delete(`http://localhost:8001/oportunidades/${registroSelecionado?.id}/`);
            setModo("list");
            setModoForm(null);
            setRegistroSelecionado(null);
            setFormTipo(null);
  ...
  <div className="text-red-600">{erroOportunidades}</div>
            ...
            <DataGrid
             ...
             // pinnedColumns={{ right: ["acoes"] }} // <- coluna fixa à direita Só funciona na Grid Pro / Premium
              ...
              sx={{
                ...
                boxShadow: "0 4px 8px rgba(241, 218, 218, 0.48)",
        ...
        {modo === "list" && (
        <Box sx={{width: 300, mx: "right" }}>
         <Typography gutterBottom>Ajustar altura das linhas</Typography>
            <Slider
              min={30}
              max={50}
              step={1}
              value={rowHeight}
              onChange={(e, newValue) => setRowHeight(newValue)}
        ...
        {modo === "form" && formTipo === "aprovacao" && ( 
         <Box> <Typography> <h1></h1></Typography></Box>
        )}
